...
@app.get("/search")
def search_songs(query: str = Query(..., description="Search query"), limit: int = Query(10, description="Number of results to return"), scope: str = Query(None, description="Search scope/filter")):
    start_time = time.time()
    try:
        # Determine the filter based on the scope provided
        search_filter = None
        if scope and scope.lower() in ["songs", "artists", "albums", "playlists"]:
            search_filter = scope.lower()
        elif scope and scope.lower() == "all":
            search_filter = None

        if search_filter:
             search_results = ytmusic.search(query, filter=search_filter, limit=limit)
        else:
            # Default fallback if 'all' or no scope provided
        ...
        if not search_results:
            logger.info(f"No results for '{query}', trying fallback query 'top hits'")
            search_results = ytmusic.search("top hits", filter="songs", limit=limit)
        
        # Prefetch the top few results in the background with HIGH priority (only for songs)
        if search_filter == "songs" or search_filter is None:
...
@app.get("/artist")
def get_artist(browseId: str = Query(..., description="Artist Browse ID")):
    try:
        logger.info(f"Fetching artist details for browseId: {browseId}")
        artist_details = ytmusic.get_artist(browseId)
...
def get_album_details(browse_id: str):
    """Get complete details and tracks for a specific album"""
    try:
        logger.info(f"Fetching album with ID: {browse_id}")
        album_data = ytmusic.get_album(browse_id)
        
        # Prefetch top tracks in the background
        if album_data and 'tracks' in album_data:
            video_ids = [track.get('videoId') for track in album_data['tracks'][:3] if track.get('videoId')]
            if video_ids:
                background_prefetch_audio_urls(video_ids, TaskPriority.MEDIUM)
                
        return album_data
    except Exception as e:
        logger.error(f"Error fetching album {browse_id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get album details: {str(e)}")
